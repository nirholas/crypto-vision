# XTools CLI Reference

Complete reference for all XTools CLI commands.
...
**Example:**
```bash
xtools scrape profile elonmusk -o elon.json
```
...
```bash
xtools scrape replies TWEET_URL [OPTIONS]
...
```bash
xtools scrape thread TWEET_URL [OPTIONS]
...
```bash
xtools scrape hashtag HASHTAG [OPTIONS]
...
```bash
xtools follow user USERNAME [OPTIONS]
...
**Example:**
```bash
xtools follow by-keyword "AI" "machine learning" --max 50 --min-followers 500
```
...
```bash
xtools follow by-hashtag HASHTAG [OPTIONS]
...
```bash
xtools follow engagers TWEET_URL [OPTIONS]
...
```bash
xtools unfollow user USERNAME [OPTIONS]
...
**Example:**
```bash
xtools unfollow non-followers --whitelist friend1 --whitelist friend2 --dry-run
```
...
```bash
xtools unfollow inactive [OPTIONS]
...
```bash
xtools unfollow by-criteria [OPTIONS]
...
```bash
xtools engage like TWEET_URL [OPTIONS]
```
...
```bash
xtools engage unlike TWEET_URL
```
...
```bash
xtools engage auto-like KEYWORDS... [OPTIONS]
...
```bash
xtools engage comment TWEET_URL [OPTIONS]
...
```bash
xtools engage auto-comment KEYWORDS... [OPTIONS]
...
```bash
xtools engage retweet TWEET_URL [OPTIONS]
...
```bash
xtools engage auto-engage USERNAME [OPTIONS]
...
```bash
xtools monitor new-followers [OPTIONS]
...
```bash
xtools monitor mentions [OPTIONS]
...
```bash
xtools ai reply TWEET_TEXT [OPTIONS]
...
**Example:**
```bash
xtools ai reply "Just launched my AI startup!" --style witty --num 3
```
...
```bash
xtools ai generate TOPIC [OPTIONS]
...
**Example:**
```bash
xtools ai generate "Python tips" --thread --num 7
```
...
```bash
xtools ai sentiment TEXT [OPTIONS]
...
```bash
xtools ai analyze-user USERNAME [OPTIONS]
...
```bash
xtools ai improve TEXT [OPTIONS]
...
```bash
xtools ai find-targets NICHE [OPTIONS]
...
```bash
xtools ai crypto-sentiment TOKEN [OPTIONS]
...
**Example:**
```bash
xtools ai crypto-sentiment BTC --detailed
```
...
```bash
# Export as JSON
xtools scrape followers user -o followers.json -f json

# Export as CSV
xtools scrape tweets user -o tweets.csv -f csv
```
...
| `XTOOLS_DEBUG` | Enable debug mode |
