# XTools Features Guide

Comprehensive guide to all XTools features.
...
```bash
# CLI
xtools scrape profile elonmusk -o profile.json
...
```bash
# Get followers
xtools scrape followers elonmusk --limit 1000 -o followers.json

# Get following
xtools scrape following elonmusk --limit 1000
```
...
```bash
# User timeline
xtools scrape tweets elonmusk --limit 100 --include-replies

# Replies to a tweet
xtools scrape replies https://twitter.com/user/status/123

# Thread
xtools scrape thread https://twitter.com/user/status/123
```
...
```bash
# Search tweets
xtools scrape search "AI machine learning" --type tweets

# Search users
xtools scrape search "AI developer" --type users
```
...
```bash
# By keyword
xtools follow by-keyword "Python" "JavaScript" --max 50 --min-followers 500

# Followers of competitor
xtools follow followers-of competitor_account --max 100

# Engagers on a tweet
xtools follow engagers tweet_url --likers --commenters
```
...
xtools unfollow non-followers --whitelist friend1 friend2 --dry-run

# Inactive accounts
xtools unfollow inactive --days 90

# Smart unfollow
xtools unfollow smart --target-ratio 1.2 --preserve-engagement
```
...
xtools engage auto-like "AI" "ML" --max 50 --duration 30 --dry-run
```
...
xtools engage auto-comment "startup" "founder" --style helpful --max 20
```
...
xtools engage auto-engage target_user --likes 5 --comments 2 --retweets 1
```
...
# Create snapshot
xtools monitor unfollowers/snapshot
```
...
```bash
# Growth report
xtools monitor growth --period 30d -o growth_report.json
```
...
```bash
# Monitor topics
xtools monitor keywords "brand" "product" --alert-threshold 10 --sentiment
```
...
```bash
# Monitor mentions
xtools monitor mentions --continuous --interval 5 --notify
```
...
```bash
xtools follow by-keyword "AI" --max 50 --dry-run
```
...
```bash
xtools scrape followers user --limit 1000 -o followers.csv -f csv
```
