...
xtools scrape profile competitor -o competitor_profile.json

# 2. Scrape their followers (potential targets)
xtools scrape followers competitor --limit 1000 -o competitor_followers.csv -f csv

# 3. Scrape their popular tweets
xtools scrape tweets competitor --limit 50 -o competitor_tweets.json
```
...
xtools follow by-keyword "Python" "data science" \
  --max 25 \
  --min-followers 500 \
  --max-followers 50000 \
  --dry-run
...
xtools engage auto-like "Python tips" "coding" \
  --max 30 \
  --duration 60
```
...
xtools monitor growth --period 7d -o weekly_growth.json
```
...
# Usage
client = XToolsClient(api_key="your-api-key")
...
```python
from xtools.ai.providers import ProviderError, RateLimitError, AuthenticationError
