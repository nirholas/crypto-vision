...
FastAPI-based REST API for XTools functionality.
"""
...
class Settings:
    ...
    API_KEY: str = os.getenv("XTOOLS_API_KEY", "")
    DEBUG: bool = os.getenv("XTOOLS_DEBUG", "false").lower() == "true"
    RATE_LIMIT: int = int(os.getenv("XTOOLS_RATE_LIMIT", "60"))
    CORS_ORIGINS: list[str] = os.getenv("XTOOLS_CORS_ORIGINS", "*").split(",")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("XTools API starting up...")
    yield
    logger.info("XTools API shutting down...")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="XTools API",
        description="""
# XTools - X/Twitter Automation Toolkit API

⚠️ **EDUCATIONAL PURPOSES ONLY** - Do not run against X/Twitter.

This API provides programmatic access to XTools functionality including:
